...
def _fallback_dashboard_metrics(config: AppConfig, limit: int) -> dict[str, list[dict[str, Any]]]:
    ...
    hardest = (
        ...
        .agg(
            ...
            attempts=("score_numeric", lambda series: int(series.notna().sum())),
            events=("topic", "size"),
        )
        .reset_index()
    )
...
def dashboard_metrics(config: AppConfig | None = None, limit: int = 10) -> dict[str, list[dict[str, Any]]]:
    """Return analytics sections from Spark when explicitly enabled, otherwise use local JSON log analytics."""
    app_config = ensure_data_directories(config or get_config())
    use_spark = os.getenv("LEARNMATE_USE_SPARK_ANALYTICS", "false").lower() == "true"
    if F is None or not use_spark:
    ...
    try:
        ...
        top_df = (
            ...
            .agg(
                F.round(F.avg("score"), 2).alias("avg_score"),
                F.count("*").alias("attempts"),
                F.round(F.max("score"), 2).alias("best_score"),
            )
