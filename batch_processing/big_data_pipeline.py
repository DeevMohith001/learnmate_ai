...
def _read_event_lake(spark, config: AppConfig) -> DataFrame:
    ...
    if not raw_events_root.exists():
        return _empty_log_frame(spark)
    json_files = list(raw_events_root.rglob('*.json'))
    if not json_files:
        return _empty_log_frame(spark)
...
def load_log_dataframes(spark, config: AppConfig | None = None) -> dict[str, DataFrame]:
    ...
    logs['event_lake'] = _read_event_lake(spark, app_config)
    return logs
...
def persist_bronze_zone(logs: dict[str, DataFrame], tables: dict[str, DataFrame], config: AppConfig) -> dict[str, str]:
    ...
    for name, frame in logs.items():
        ...
        frame.write.mode('overwrite').json(target)
...
def build_silver_events(logs: dict[str, DataFrame], tables: dict[str, DataFrame]) -> dict[str, DataFrame]:
    ...
    chat_messages_df = (
        ...
        .join(tables['chat_sessions'].select(F.col('id').alias('session_join_id'), F.col('topic').alias('session_topic'), F.col('document_id').alias('session_document_id')), tables['chat_messages']['session_id'] == F.col('session_join_id'), 'left')
        .withColumn('event_timestamp', F.to_timestamp('created_at'))
...
def build_gold_tables(silver: dict[str, DataFrame]) -> dict[str, DataFrame]:
    ...
    user_engagement = (
        ...
        .agg(
            F.count('*').alias('total_events'), F.round(F.avg('score'), 2).alias('avg_score'),
            F.round(F.avg('engagement_score'), 2).alias('avg_engagement'), F.sum('duration_seconds').alias('total_duration_seconds'),
...
def run_batch_pipeline(config: AppConfig | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        report = {
            ...
            'recommendation_features_preview': [row.asDict() for row in gold['recommendation_features'].limit(15).collect()],
        }
